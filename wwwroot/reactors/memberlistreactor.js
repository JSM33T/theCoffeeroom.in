const MemberDashboard = {
    ...
    setup(props) {
        ...
        const fetchMemberData = async () => {
            try {
                startProg();
                const response = await axios.get('/api/member/getinfo/' + endPoint.value);
                members.value = response.data;
                console.log('/api/members/getmembers/' + param1.value);
                console.log(response.data);
            } catch (error) {
                 ...
                 <div v-for="member in members" :key="member.userName" class="col-md-6 pb-2 pt-2">
                    <div class="card h-100 rounded-3 p-3 p-sm-4">
                      <div class="d-flex align-items-center pb-2 mb-1">
                        <h3 class="h6 text-nowrap text-truncate mb-0"><router-link :to="'/member/' + member.userName ">@{{ member.userName }}</router-link></h3>z
